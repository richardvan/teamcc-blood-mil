...
def full_report(model, organized_dir, patient_ids, labels, image_ext, device, name):
    ...
    print(report)
    # Structured (dict) version too, so per-class recall/F1 can be averaged
    # across CV folds later -- the text report alone can't be aggregated.
    ...
    return cm, report, report_dict, bal_acc
...
def main():
    ...
    val_cm, val_report, val_report_dict, val_bal_acc = full_report(
        model, args.organized_dir, val_ids, labels, args.image_ext, device, f"Validation (fold {args.fold})")
    hold_cm, hold_report, hold_report_dict, hold_bal_acc = full_report(
        model, args.organized_dir, holdout_ids, labels, args.image_ext, device, "Holdout (final)")
    ...
    with open(os.path.join(args.model_dir, f"holdout_report_{tag}.json"), "w") as f:
        json.dump({
            ...
            "holdout_per_class_report": hold_report_dict,
            "val_per_class_report": val_report_dict,
            "class_order": CLASSES,
            "unfreeze_from": args.unfreeze_from,
            "instances_per_step": args.instances_per_step,

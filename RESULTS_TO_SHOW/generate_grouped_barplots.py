"""
Parse the per-fold multiclass classification_report*.txt files in this folder
and generate grouped bar plots comparing models, with error bars = std dev
across the 5 CV folds.

Only uses the NON-holdout, NON-binary files (i.e. files that have 5-fold
"=== Fold N ===" blocks), since those are the only ones with enough repeats
to compute a mean +/- std dev. Holdout files get a separate script later
(single point estimate per model, no std dev possible).

Usage:
    python generate_grouped_barplots.py
Outputs (written to this folder):
    barplot_f1_by_class.png
    barplot_accuracy_by_model.png
    barplot_macro_f1_by_model.png
"""
...
OUTPUT_FOLDER = os.path.join(FOLDER, "grouped_barplots")
...
def load_all():
    ...
    for path in find_multiclass_files():
...
def grouped_bar_with_error(
    ...
    a fixed color (used so e.g. class colors match the PCA plot regardless
    of which axis holds the model vs. the class).
    """
...
def main():
    ...
    if class_df.empty:
        print("No multiclass fold data found -- nothing to plot.")
        return
    ...
    # Bars are colored by class (matching the PCA plot's class colors); the
    # x-axis holds the models, with one bar per class within each cluster.
    ...
    ax.set_title("Per-class F1-score by model (mean ± std across 5 folds)")
    ax.set_ylim(0, 1.05)
    ax.legend(title="Class", bbox_to_anchor=(1.02, 1), loc="upper left")
    fig.tight_layout()
    fig.savefig(os.path.join(OUTPUT_FOLDER, "barplot_f1_by_class.png"), dpi=200)
    plt.close(fig)
    ...
    if not accuracy_df.empty:
        ...
        ax.set_ylim(0, 1.05)
        ax.set_title("Overall accuracy by model (mean ± std across 5 folds)")
        fig.tight_layout()
        fig.savefig(os.path.join(OUTPUT_FOLDER, "barplot_accuracy_by_model.png"), dpi=200)
        plt.close(fig)
        ...
        if "macro_f1" in accuracy_df.columns:
            ...
            ax.set_ylim(0, 1.05)
            ax.set_title("Macro-avg F1-score by model (mean ± std across 5 folds)")
            fig.tight_layout()
            fig.savefig(os.path.join(OUTPUT_FOLDER, "barplot_macro_f1_by_model.png"), dpi=200)
            plt.close(fig)

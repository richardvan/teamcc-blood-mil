...
label_categories = ["normal", "cancer"]                                 ## NEW CODE (label 0/1 names, mirrors label_categories in the multiclass script)
...
pipe = Pipeline([
    ...
    ("svm", SVC(class_weight="balanced", probability=True)),           ## NEW CODE (probability=True enables predict_proba for AUROC)
])
...
import matplotlib.colors                                                  ## NEW CODE
...
colors = np.array([                                                     ## NEW CODE (shared with the ROC plot below so each class's color matches across plots)
    plt.cm.tab10(np.linspace(0, 1, 5))[4],                              ## NEW CODE (normal: same color as "normal.control" in the multiclass script)
    matplotlib.colors.to_rgba("tab:orange"),                            ## NEW CODE (cancer: distinct from normal)
])
for class_idx, class_name in enumerate(label_categories):              ## NEW CODE (loop over classes instead of two hardcoded scatter calls)
  plt.scatter(Z_pca[y_patient==class_idx,0], Z_pca[y_patient==class_idx,1], label=class_name, alpha=.6, c=[colors[class_idx]])  ## NEW CODE
plt.legend(title="train")                                                ## NEW CODE (titled to match the holdout eval script's train/holdout split legend)
plt.title("Patient embeddings (PCA)")
...
if len(label_categories) == 2:                                           ## NEW CODE (label_binarize collapses binary labels to a single column)
  y_bin_all = np.hstack([1 - y_bin_all, y_bin_all])                       ## NEW CODE
...
mean_fpr = np.linspace(0, 1, 100)                                         ## NEW CODE (reuses `colors` defined above in the PCA plot section, so class colors match across plots)
...
fig, axes = plt.subplots(1, len(label_categories) + 1, figsize=(5 * (len(label_categories) + 1), 5))  ## NEW CODE (1 summary panel + 1 panel per class)
...
summary_ax.set_title("All classes (mean ROC)")                          ## NEW CODE
summary_ax.legend(fontsize=7)                                            ## NEW CODE

fig.suptitle("ROC curves (CV, per-fold + mean per class)")              ## NEW CODE
...
N_CLASSES = len(label_categories)                                          ## NEW CODE
cm = confusion_matrix(y_train, pred, labels=range(N_CLASSES))              ## NEW CODE
cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True).clip(min=1)      ## NEW CODE

fig, ax = plt.subplots(figsize=(7, 6))                                     ## NEW CODE
im = ax.imshow(cm_norm, cmap='Blues', vmin=0, vmax=1)                      ## NEW CODE
ax.set_xticks(range(N_CLASSES)); ax.set_yticks(range(N_CLASSES))           ## NEW CODE
ax.set_xticklabels(label_categories, rotation=45, ha='right')              ## NEW CODE
ax.set_yticklabels(label_categories)                                       ## NEW CODE
ax.set_xlabel('predicted'); ax.set_ylabel('true')                          ## NEW CODE

for i in range(N_CLASSES):                                                 ## NEW CODE
    for j in range(N_CLASSES):                                             ## NEW CODE
        ax.text(j, i, f'{cm[i,j]}\n({cm_norm[i,j]:.2f})', ha='center', va='center',  ## NEW CODE
                color='white' if cm_norm[i,j] > 0.5 else 'black', fontsize=9)  ## NEW CODE

plt.colorbar(im, fraction=0.046, pad=0.04)                                 ## NEW CODE
plt.title('Confusion Matrix (5-fold CV)')                                  ## NEW CODE
plt.tight_layout()                                                         ## NEW CODE
CM_PLOT_PATH = os.path.join(SAVE_DIR, 'confusion_matrix_heatmap.png')      ## NEW CODE
plt.savefig(CM_PLOT_PATH, bbox_inches='tight')                             ## NEW CODE
print("saved confusion matrix heatmap to", CM_PLOT_PATH)                   ## NEW CODE
plt.show()                                                                 ## NEW CODE
